...
const CopyTemplatePage = ({ params }: { params: { slug: string } }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user?.emailAddresses[0].emailAddress && template) {
      const { id, ...templateWithoutId } = template // id 필드 제거
      const newTemplate = {
        ...templateWithoutId,
        slug,
        category,
      ...
      try {
        await db.insert(UserTemplates).values(newTemplate)
        router.push('/dashboard/my-templates')
      } catch (error) {
        console.error('템플릿 저장 중 오류 발생:', error)
        alert('템플릿 저장 중 오류가 발생했습니다. 다시 시도해 주세요.')
      }
    }
  }
  ...
  return (
    <div className='p-5'>
      <h1 className='text-2xl font-bold mb-4'>템플릿 복사</h1>
